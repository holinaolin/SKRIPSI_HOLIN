...
def run_motor(motor, _id, _rpm):
    motor.set_drive_mode(_id=_id, _mode=2)
    motor.send_rpm(_id, _rpm)
    time.sleep(3)  # Berjalan selama 3 detik
    motor.send_rpm(_id, 0)  # Menghentikan motor

class PIDController:
    def __init__(self, kp, ki, kd):
        self.kp = kp  # Koefisien proporsional
        self.ki = ki  # Koefisien integral
        self.kd = kd  # Koefisien derivatif
        self.previous_error = 0  # Error sebelumnya, untuk perhitungan derivatif
        self.integral = 0  # Kumulatif error, untuk perhitungan integral

    def calculate(self, setpoint, measured_value):
        """Menghitung output kontrol PID."""
...
def adjust_heading(pid, motor, motor2, current_orientation):
    # Target heading adalah 0 derajat
    rpm_adjustment = pid.calculate(0, current_orientation)
    
    # Asumsi output PID mempengaruhi selisih RPM antar motor
    # Misal, jika rpm_adjustment positif, motor1 harus lebih lambat daripada motor2, dan sebaliknya
    # Maksimum RPM diset ke 30 untuk kedua motor
    rpm1 = max(-30, min(30, -rpm_adjustment))
    rpm2 = max(-30, min(30, rpm_adjustment))
    
    # Estimasi durasi untuk penyesuaian, bisa disesuaikan berdasarkan pengujian
    t = 2  # durasi dalam detik, nilai ini hanya sebagai contoh

    # Mengirim perintah RPM ke motor
    motor.send_rpm(1, rpm1)
    motor2.send_rpm(1, rpm2)
    
    # Menjalankan motor selama 't' detik
    time.sleep(t)
    
    # Menghentikan motor setelah waktu berlalu
    motor.send_rpm(1, 0)
    motor2.send_rpm(1, 0)

if __name__ == "__main__":
	motor = MotorControl(device="COM3")
	motor2 = MotorControl(device="COM5")

	pid = PIDController(kp=1.0, ki=0.1, kd=0.05)
	current_orientation = 20
	adjust_heading(pid, motor, motor2, current_orientation)

	
    # # Sesuaikan nilai X, Y, dan t berdasarkan pengujian dan kalibrasi
    # X = 30  # Nilai contoh untuk keperluan demonstrasi
    # Y = 0  # Nilai contoh untuk keperluan demonstrasi
    # t = 2.5   # Nilai contoh untuk keperluan demonstrasi
    
    # Memanggil fungsi untuk menyesuaikan heading
    # adjust_heading_to_zero(motor, motor2)
    
    # rpm1, rpm2, t = adjust_heading(pid, current_orientation)
    # print(f"Set RPM Motor1: {rpm1}, RPM Motor2: {rpm2}, Durasi: {t} detik")

    # # Parameter PID (kp, ki, kd) perlu disesuaikan berdasarkan pengujian
    # while True:
    #     # Mendapatkan orientasi saat ini dari input pengguna
    #     current_orientation = float(input("Masukkan orientasi saat ini (derajat): "))
        
    #     # Jika orientasi sangat dekat dengan 0, berhenti
    #     if abs(current_orientation) < 1:
    #         print("Orientasi sudah cukup dekat dengan 0 derajat.")
    #         break
        
    #     # Menyesuaikan orientasi berdasarkan kontrol PID
    #     adjust_heading_to_zero(motor, motor2, pid, current_orientation=current_orientation)

    # # Menghentikan motor setelah orientasi sesuai target
    # motor.send_rpm(1, 0)
    # motor2.send_rpm(1, 0)

    # # Jalankan kedua motor secara paralel
    # t1 = threading.Thread(target=run_motor, args=(motor, 1, 50))
    # t2 = threading.Thread(target=run_motor, args=(motor2, 1, -50))
